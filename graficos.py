...
for metrica in metricas:
    ...
    df_bm25 = pd.read_csv('./output/bm25_scifact.csv')
    df_sbert_cos = pd.read_csv('./output/sbert_cos_sim.csv')
    df_sbert_dot = pd.read_csv('./output/sbert_dot.csv')
    ...
    ax.set_xlabel('')
    ax.set_title('Desempenho dos Algoritmos usando a métrica '+metrica.upper()+' do dataset ')
    ax.set_xticks(x)
    ...
    plt.savefig('./output/comparacao_algoritmos_'+metrica+'.png')
    plt.show()
